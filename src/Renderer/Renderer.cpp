...
bool Renderer::renderProject(const Project& project, const QString& output, QString* error)
{
    ...
    if (!QFileInfo(m_resampler).isFile()) {
    ...
    for (int trackIndex = 0; trackIndex < project.tracks().size(); ++trackIndex) {
        const auto& track = project.tracks().at(trackIndex);
        ...
        if (!singer->isValid()) {
            if (error) *error = QStringLiteral("VoiceBank '%1' cannot be loaded: %2").arg(singer->info().name, singer->oto().error());
            return false;
        }
        ...
        for (const auto& phoneme : phonemes) {
            ...
            if (!oto) {
                if (error) *error = QStringLiteral("Alias '%1' was not found in VoiceBank '%2'.").arg(phoneme.alias, singer->info().name);
                return false;
            }
            ...
            request.volume = std::clamp(track.volume() * note.getIntensity() / 100.0, 0.0, 2.0) * 100.0;
            ...
            segment.startMs = qRound64(tickToMs(project.tempoMap(), static_cast<qint64>(phoneme.startTick), project.ppq()));
            segment.lengthMs = qRound64(renderLengthMs);
            segment.gain = std::clamp(track.volume(), 0.0, 2.0);
            segment.overlapMs = std::max(0.0, phoneme.overlap);
