...
{
    return static_cast<quint16>(static_cast<quint8>(b.at(p)) | (static_cast<quint16>(static_cast<quint8>(b.at(p + 1))) << 8));
}
...
double sampleToFloat(const QByteArray& bytes, qsizetype p, int bits, int format)
{
    ...
    case 16: return static_cast<qint16>(rd16(bytes, p)) / 32768.0;
    case 24: {
        ...
        const qint32 signedValue = (raw & 0x00800000u) ? static_cast<qint32>(raw | 0xff000000u) : static_cast<qint32>(raw);
        return signedValue / 8388608.0;
    }
    case 32: return static_cast<qint32>(rd32(bytes, p)) / 2147483648.0;
    default: return 0.0;
    }
}
}
...
bool AudioEngine::loadPcmWav(const QString& path)
{
    ...
    while (pos + 8 <= bytes.size()) {
        ...
        if (tag(bytes, pos, "fmt ") && safeSize >= 16) {
            ...
            if (format == 0xfffe && safeSize >= 40) {
                ...
                if (subFormat == 1) format = 1; else if (subFormat == 3) format = 3;
            }
    ...
    if ((format != 1 && format != 3) || channels <= 0 || channels > 32 || rate <= 0 ||
        (bits != 8 && bits != 16 && bits != 24 && bits != 32) || dataOffset < 0 || dataSize <= 0) return false;
    ...
    m_pcm.resize(frames * channels * 2);
...
void AudioEngine::load(const QString& path)
{
    destroySink();
    destroyPrimaryPlayer();
    m_loaded = false; m_primaryIsPcm = false; m_pcm.clear(); m_seekMs = 0; m_seekByte = 0;
    ...
    connect(m_primaryPlayer.get(), &QMediaPlayer::errorOccurred, this, [this](QMediaPlayer::Error, const QString& text) {
...
void AudioEngine::startClock(qint64 baseMs) { m_clockBaseMs = std::max<qint64>(0, baseMs); m_clock.start(); m_positionTimer.start(); }
void AudioEngine::stopClock() { if (m_clock.isValid()) m_clockBaseMs = std::max<qint64>(0, position()); m_clock.invalidate(); m_positionTimer.stop(); }
    ...
    if (m_loaded && m_primaryIsPcm) {
        if (!m_sink) {
            ...
            connect(m_sink.get(), &QAudioSink::stateChanged, this, [this](QAudio::State state) {
                ...
                if (state == QAudio::IdleState) { stopClock(); emit playbackStateChanged(false); }
            });
        }
        if (m_sink->state() == QAudio::SuspendedState) m_sink->resume();
        else { m_seekByte = m_buffer.pos(); m_sink->start(&m_buffer); }
    } else if (m_loaded && m_primaryPlayer) {
        m_primaryPlayer->setPosition(base); m_primaryPlayer->play();
    }
    syncBackingPlayers(base, true); startClock(base); emit playbackStateChanged(true);
}
...
{
    ms = std::max<qint64>(0, ms); const bool wasPlaying = isPlaying();
    if (m_primaryIsPcm && m_loaded && m_format.sampleRate() > 0 && m_format.channelCount() > 0) {
        ...
        byte = std::clamp<qint64>(byte, 0, m_pcm.size()); m_buffer.seek(byte); m_seekMs = ms; m_seekByte = byte;
    } else if (m_primaryPlayer) { m_primaryPlayer->setPosition(ms); m_seekMs = ms; }
    else m_seekMs = ms;
    if (wasPlaying) startClock(ms); else if (!m_primaryIsPcm) m_clockBaseMs = ms;
    syncBackingPlayers(ms, wasPlaying); emit positionChanged(ms); if (wasPlaying && m_primaryIsPcm) play();
}
bool AudioEngine::isPlaying() const
{
    ...
    for (const auto& player : m_backingPlayers) if (player && player->playbackState() == QMediaPlayer::PlayingState) return true;
    return m_clock.isValid();
}
...
{
    midi = std::clamp(midi, 0, 127); durationMs = std::clamp(durationMs, 30, 250);
    constexpr int rate = 44100; const int frames = rate * durationMs / 1000; m_previewPcm.resize(frames * 2);
    ...
    for (int i = 0; i < frames; ++i) {
        ...
        const double sample = std::sin(2.0 * pi * frequency * t) * std::min(attack, release) * 0.22;
        const qint16 value = static_cast<qint16>(std::clamp(sample, -1.0, 1.0) * 32767.0);
        m_previewPcm[i * 2] = static_cast<char>(value & 0xff); m_previewPcm[i * 2 + 1] = static_cast<char>((value >> 8) & 0xff);
    }
    if (m_previewSink) m_previewSink->stop(); if (m_previewBuffer.isOpen()) m_previewBuffer.close();
    m_previewFormat = QAudioFormat(); m_previewFormat.setSampleRate(rate); m_previewFormat.setChannelCount(1); m_previewFormat.setSampleFormat(QAudioFormat::Int16);
    m_previewBuffer.setData(m_previewPcm); m_previewBuffer.open(QIODevice::ReadOnly); m_previewSink = std::make_unique<QAudioSink>(m_previewFormat); m_previewSink->setVolume(0.7); m_previewSink->start(&m_previewBuffer);
}
