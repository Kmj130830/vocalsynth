...
void AudioSpectrumWidget::refresh()
{
    ...
    if (source == m_sourcePath && m_decoder->sourceUrl().isValid()) {
        update();
        return;
    }
    decodeFile(source);
...
void AudioSpectrumWidget::readBuffer()
{
    ...
    for (int offset = 0; offset + kFftSamples <= mono.size(); offset += kHopSamples) {
        ...
        for (int band = 0; band < kBands; ++band) {
            ...
            const double omega = 2.0 * M_PI * frequencyBin / kFftSamples;
            double real = 0.0;
            double imag = 0.0;
            for (int n = 0; n < kFftSamples; ++n) {
                const double window = 0.5 - 0.5 * std::cos(2.0 * M_PI * n / (kFftSamples - 1));
                const double sample = mono[offset + n] * window;
                real += sample * std::cos(omega * n);
                imag -= sample * std::sin(omega * n);
            }
            const double magnitude = std::sqrt(real * real + imag * imag) / kFftSamples;
            bands[band] = static_cast<float>(std::clamp(magnitude * 7.0, 0.0, 1.0));
        }
...
void AudioSpectrumWidget::paintEvent(QPaintEvent*)
{
    ...
    for (int i = 0; i < m_columns.size(); ++i) {
        ...
        const double columnGain = 0.75 + 0.25 * std::sin(i * 0.07);
        for (int b = 0; b < bands.size(); ++b) {
            const float level = std::clamp(static_cast<float>(bands.at(b) * columnGain), 0.0f, 1.0f);
            const int barHeight = std::max(1, static_cast<int>(level * usableHeight * 0.72));
            const int y = height() - 5 - barHeight;
            p.fillRect(x, y, std::max(1, columnWidth - 1), barHeight, QColor("#485a70"));
        }
    }

    // Playback cursor is drawn in exactly the same millisecond coordinate space as Arrangement.
    qint64 totalMs = 0;
    if (m_project) {
        for (const auto& clip : m_project->audioClips()) {
            if (clip.path == m_sourcePath) totalMs = std::max(totalMs, clip.durationMs);
        }
    }
    if (totalMs > 0) {
        const double u = std::clamp(static_cast<double>(m_playheadMs) / totalMs, 0.0, 1.0);
        const int x = qRound(u * width());
