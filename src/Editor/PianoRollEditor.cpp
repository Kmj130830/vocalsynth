...
void PianoRollEditor::paintEvent(QPaintEvent*)
{
    ...
    if (m_project && m_activeTrack >= 0 && m_activeTrack < static_cast<int>(m_project->tracks().size())) {
        for (const auto& note : m_project->tracks()[m_activeTrack].notes()) {
            ...
            if (note.isSelected()) { p.setPen(QColor("#e0ebff")); p.drawLine(r.right() - 3, r.top() + 3, r.right() - 3, r.bottom() - 3); }
        }
...
void PianoRollEditor::mousePressEvent(QMouseEvent* event)
{
    ...
    if (event->button() == Qt::RightButton) {
        ...
        m_project->tracks()[m_activeTrack].addNote(std::move(note));
        m_rightDrawStart = start; m_rightDrawPitch = midiAtY(pressPos.y()); m_rightDrawId = m_project->tracks()[m_activeTrack].notes().back().getId(); m_rightDrawing = true;
...
void PianoRollEditor::mouseMoveEvent(QMouseEvent* event)
{
    ...
    if (m_rightDrawing && m_rightDrawId >= 0) {
        if (Note* note = m_project->tracks()[m_activeTrack].findNote(m_rightDrawId)) {
            ...
            for (const auto& other : m_project->tracks()[m_activeTrack].notes()) if (other.getId() != note->getId() && other.getStartTick() > m_rightDrawStart) end = std::min(end, other.getStartTick());
    ...
    if (m_resizing) {
        if (Note* note = m_project->tracks()[m_activeTrack].findNote(m_resizeId)) {
            ...
            note->setDurationTick(std::max<qint64>(1, availableEndForNote(*note, desired) - note->getStartTick())); emit documentChanged(); viewport()->update();
        }
        return;
    }
    ...
    sortNotes(); emit documentChanged(); viewport()->update();
}
...
void PianoRollEditor::keyPressEvent(QKeyEvent* event)
{
    ...
    if (event->key() == Qt::Key_Escape && m_lyricEditor) { finishLyricEdit(false); return; }
...
void PianoRollEditor::createNote(const QPoint& pos)
{
    ...
    if (end <= start) return;
    for (auto& n : m_project->tracks()[m_activeTrack].notes()) n.setSelected(false);
    Note note(QDateTime::currentMSecsSinceEpoch()); note.setStartTick(start); note.setDurationTick(end - start); note.setMidiNote(midiAtY(pos.y())); note.setLyric(QStringLiteral("a")); note.setSelected(true);
...
void PianoRollEditor::createDraggedNote(qint64 startTick, qint64 endTick, int midi)
{
    ...
    Note note(QDateTime::currentMSecsSinceEpoch()); note.setStartTick(startTick); note.setDurationTick(endTick - startTick); note.setMidiNote(std::clamp(midi, 0, 127)); note.setLyric(QStringLiteral("a")); note.setSelected(true);
...
void PianoRollEditor::finishLyricEdit(bool accept)
{
    ...
    if (accept && m_project) if (Note* n = m_project->tracks()[m_activeTrack].findNote(id)) if (n->getLyric() != text) { n->setLyric(text); emit documentChanged(); }
