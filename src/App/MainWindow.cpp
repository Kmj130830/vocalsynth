      ...
      m_renderer(&m_singers, this)
{
    ...
    if (m_resampler.isEmpty()) {
        statusBar()->showMessage(QStringLiteral("Moresampler not found. Set it in Tools > Preferences."), 6000);
    }
    ...
    m_phonemeStrip->setActiveTrack(0);
    resize(1440, 930);
    updateTitle();
}
...
void MainWindow::buildUi()
{
    ...
    m_phonemeStrip = new PhonemeStripEditor(m_project.get(), this);
    m_phonemeStrip->setPixelsPerSecond(90.0);

    auto* editorSplitter = new QSplitter(Qt::Vertical, this);
    editorSplitter->setChildrenCollapsible(false);
    editorSplitter->addWidget(m_arrangement);
    editorSplitter->addWidget(pianoContainer);
    editorSplitter->addWidget(m_phonemeStrip);
    editorSplitter->setStretchFactor(0, 2);
    editorSplitter->setStretchFactor(1, 7);
    editorSplitter->setStretchFactor(2, 1);
    editorSplitter->setSizes({170, 600, 120});
...
void MainWindow::buildMenus()
{
    ...
    edit->addAction(QStringLiteral("Undo"), this, [this] {
        ...
        m_phonemeStrip->viewport()->update();
    });
...
void MainWindow::connectUi()
{
    ...
    connect(m_toolbar, &MainToolBar::snapToggled, this, [this](bool enabled) {
        m_project->setSnapEnabled(enabled);
        m_editor->setSnapEnabled(enabled);
    });
    connect(m_toolbar, &MainToolBar::gridToggled, this, [this](bool enabled) {
        m_project->setGridVisible(enabled);
        m_editor->setShowGrid(enabled);
    });
    connect(m_toolbar, &MainToolBar::gridResolutionChanged, this, [this](int div) {
        if (!m_project) return;
        const qint64 ticks = qMax<qint64>(1, qRound64(m_project->ppq() * 4.0 / div));
        m_project->setGridTicks(ticks);
        m_editor->setGridTicks(ticks);
    });
    ...
    connect(m_editor, &PianoRollEditor::keyboardPreviewRequested, this,
            [this](int midi) { m_audio.previewTone(midi); });
    connect(m_keyboard, &PianoKeyboard::keyPressed, this,
            [this](int midi) { m_audio.previewTone(midi); });
    ...
    connect(m_arrangement, &ArrangementEditor::documentChanged, this, [this] {
        m_playback->invalidateCache();
        updateTitle();
    });
    ...
    connect(m_playback.get(), &PlaybackController::positionChanged, this,
            [this](qint64 ms) {
                ...
                m_phonemeStrip->setPlayheadMs(ms);
            });
    ...
    connect(m_playback.get(), &PlaybackController::playbackError, this,
            [this](const QString& message) {
                QMessageBox::critical(this, QStringLiteral("Playback failed"), message);
            });
}
...
void MainWindow::applyDefaults(Project& project)
{
    ...
    for (auto& track : project.tracks()) {
        ...
        if (!singerName.isEmpty()) {
            if (auto singer = m_singers.findByName(singerName)) {
...
void MainWindow::saveProjectAs()
{
    ...
    QString error;
    if (!m_project->save(filePath, &error)) {
        QMessageBox::critical(this, QStringLiteral("Save failed"), error);
        return;
    }
...
void MainWindow::addTrack()
{
    ...
    m_phonemeStrip->setActiveTrack(index);
    m_playback->invalidateCache();
    updateTitle();
}
...
void MainWindow::removeTrack()
{
    ...
    m_phonemeStrip->setActiveTrack(active);
    m_playback->invalidateCache();
    updateTitle();
}
...
void MainWindow::showDiagnostics()
{
    ...
    for (const auto& root : m_singers.searchRoots()) {
        text += QStringLiteral("Search root: %1\n").arg(root);
    }
...
void MainWindow::setProject(std::unique_ptr<Project> project)
{
    ...
    m_editor->setActiveTrack(0);
    m_phonemeStrip->setActiveTrack(0);
    m_editor->setSnapEnabled(m_project->snapEnabled());
    m_editor->setShowGrid(m_project->gridVisible());
    m_editor->setGridTicks(m_project->gridTicks());
    ...
    connect(m_editor, &PianoRollEditor::verticalPitchChanged, this,
            [this](int midi) { m_keyboard->setScrollPitch(midi); }, Qt::UniqueConnection);
    connect(m_editor, &PianoRollEditor::keyboardPreviewRequested, this,
            [this](int midi) { m_audio.previewTone(midi); }, Qt::UniqueConnection);
    connect(m_keyboard, &PianoKeyboard::keyPressed, this,
            [this](int midi) { m_audio.previewTone(midi); }, Qt::UniqueConnection);
    connect(m_arrangement, &ArrangementEditor::trackClicked, this, &MainWindow::selectTrack, Qt::UniqueConnection);
    connect(m_arrangement, &ArrangementEditor::positionClicked, this, &MainWindow::seekFromTimeline, Qt::UniqueConnection);
    connect(m_arrangement, &ArrangementEditor::documentChanged, this, [this] {
        m_playback->invalidateCache();
        updateTitle();
    }, Qt::UniqueConnection);
    ...
    m_phonemeStrip->setPlayheadMs(m_project->tempoMap().tickToSeconds(static_cast<double>(tick), m_project->ppq()) * 1000.0);
    updateTitle();
}
